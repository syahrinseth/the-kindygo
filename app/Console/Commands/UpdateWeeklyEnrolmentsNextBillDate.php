...
class UpdateWeeklyEnrolmentsNextBillDate extends Command
{
    protected $signature = 'enrolments:update-next-bill-date-weekly
    ...
    protected $description = 'Update next_bill_date for weekly billed enrolments';
    ...
    public function handle(ChildEnrolmentInvoiceService $invoiceService): int
    {
        ...
        $this->info('Starting next_bill_date update for WEEKLY billed enrolments...');

        $query = ChildEnrolment::query()
            ->where('billed_every', ChildEnrolmentBilledEvery::WEEKLY)
            ->whereIn('status', [
        ...
        if ($totalRecords === 0) {
            $this->info('No WEEKLY billed enrolments to process.');

            return Command::SUCCESS;
        }

        $this->info("Found {$totalRecords} WEEKLY billed enrolment(s) to process.");
