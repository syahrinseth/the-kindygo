...
class UpdateDailyEnrolmentsNextBillDate extends Command
{
    protected $signature = 'enrolments:update-next-bill-date-daily
    ...
    protected $description = 'Update next_bill_date for daily billed enrolments';
    ...
    public function handle(ChildEnrolmentInvoiceService $invoiceService): int
    {
        ...
        $this->info('Starting next_bill_date update for DAILY billed enrolments...');

        $query = ChildEnrolment::query()
            ->where('billed_every', ChildEnrolmentBilledEvery::DAILY)
            ->whereIn('status', [
        ...
        if ($totalRecords === 0) {
            $this->info('No DAILY billed enrolments to process.');

            return Command::SUCCESS;
        }

        $this->info("Found {$totalRecords} DAILY billed enrolment(s) to process.");
