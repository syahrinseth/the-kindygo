...
class UpdateYearlyEnrolmentsNextBillDate extends Command
{
    protected $signature = 'enrolments:update-next-bill-date-yearly
    ...
    protected $description = 'Update next_bill_date for yearly billed enrolments';
    ...
    public function handle(ChildEnrolmentInvoiceService $invoiceService): int
    {
        ...
        $this->info('Starting next_bill_date update for YEARLY billed enrolments...');

        $query = ChildEnrolment::query()
            ->where('billed_every', ChildEnrolmentBilledEvery::YEARLY)
            ->whereIn('status', [
        ...
        if ($totalRecords === 0) {
            $this->info('No YEARLY billed enrolments to process.');

            return Command::SUCCESS;
        }

        $this->info("Found {$totalRecords} YEARLY billed enrolment(s) to process.");
