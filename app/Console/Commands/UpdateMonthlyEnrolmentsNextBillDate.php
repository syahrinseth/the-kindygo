...
class UpdateMonthlyEnrolmentsNextBillDate extends Command
{
    protected $signature = 'enrolments:update-next-bill-date-monthly
    ...
    protected $description = 'Update next_bill_date for monthly billed enrolments';
    ...
    public function handle(ChildEnrolmentInvoiceService $invoiceService): int
    {
        ...
        $this->info('Starting next_bill_date update for MONTHLY billed enrolments...');

        $query = ChildEnrolment::query()
            ->where('billed_every', ChildEnrolmentBilledEvery::MONTHLY)
            ->whereIn('status', [
        ...
        if ($totalRecords === 0) {
            $this->info('No MONTHLY billed enrolments to process.');

            return Command::SUCCESS;
        }

        $this->info("Found {$totalRecords} MONTHLY billed enrolment(s) to process.");
